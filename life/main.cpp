...
bool update_cells(bool **array, short **neighbours, const int width,
                  const int height);
bool next_generation(bool **array, short **neighbours, const int width,
                     const int height);
void live(bool **array, short **neighbours, const int width, const int height);

int main(int argc, char **argv) {

  int width = 0, height = 0;
  std::ifstream source_file("config.txt");
  source_file >> height;
  source_file >> width;
  ...
  // создание двумерного массива для хранения количества соседей клеток в
  // динамической памяти
  ...
  // удаление двумерного массива для хранения количества соседей клеток в
  // динамической памяти
  for (int i = 0; i < height; ++i) {
    delete[] neighbours[i];
  }
...
void print_field(bool **array, const int width, const int height) {
  ...
  for (int i = 0; i < height; ++i) {
    for (int j = 0; j < width; ++j) {
      char output = array[i][j] == 0 ? Consts::DEAD : Consts::ALIVE;
      std::cout << output << ' ';
    }
    std::cout << std::endl;
  }
...
void live(bool **array, short **neighbours, const int width, const int height) {
  ...
  std::cout << "Generation: " << generation
            << ". Alive count: " << get_alive_count(array, width, height)
            << std::endl;
